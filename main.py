...
dados_padrao = {
    ...
    "rssi": -80,
    "alarme": 0,
    "tempo_porta_aberta": 0,        # Tempo em segundos
    "ultima_atualizacao": ""
}
...
<head>
    ...
    <style>
        ...
        .overlay { max-width: 700px; width: 100%; margin: auto; }
        h1 { color: #22ff66; }
        .distancia { font-size: 4.5rem; font-weight: bold; margin: 15px 0; }
        .progress-bg {
            width: 100%; background: rgba(255,255,255,0.2); height: 32px;
            border-radius: 20px; overflow: hidden; margin: 25px auto;
        }
        .progress-bar {
            height: 100%; background: linear-gradient(90deg, #22ff66, #ffaa00);
        ...
        @keyframes pisca { 0%{opacity:1;} 50%{opacity:0.4;} 100%{opacity:1;} }
    </style>
</head>
<body>
    <div class="overlay">
        <p class="titulo-projeto">Atividade Extensionista III - Projeto de Eletrônica<br>Aluno: Marcio Jose Aguiar da Silva</p>
        ...
        <p class="info" id="tempo-aberta">⏱️ Tempo Aberta: 0 segundos</p>
        ...
        function atualizarDados() {
            fetch('/dados')
                ...
                .then(data => {
                    ...
                    document.getElementById('nivel-info').textContent = `📊 Nível: ${perc}%`;

                    let status = "Aguardando...";
                    if (perc <= 25) status = "🟢 Quase Vazia";
                    else if (perc <= 60) status = "🟡 Meio Cheia";
                    else if (perc <= 85) status = "🟠 Cheia";
                    else status = "🔴 Muito Cheia - Esvaziar!";
                    document.getElementById('status').textContent = status;

                    const portaEl = document.getElementById('porta-info');
                    if (data.porta === 1) {
                        portaEl.textContent = "🚪 Porta: ABERTA";
                        if (data.alarme === 1) portaEl.classList.add('porta-alerta');
                    } else {
                        portaEl.textContent = "🚪 Porta: FECHADA";
...
def update():
    try:
        ...
        # === CORREÇÃO PRINCIPAL ===
        tempo_aberto = int(conteudo.get("tempo_porta_aberta", 0))
        ...
        novos_dados = {
            ...
            "alarme": int(conteudo.get("alarme", 0)),
            "tempo_porta_aberta": tempo_aberto,          # ← Corrigido
            "ultima_atualizacao": get_horario_brasilia()
        }

        salvar_dados(novos_dados)

        print(f"✅ Recebido → Nível: {novos_dados['nivel']}% | "
              f"Porta: {'ABERTA' if novos_dados['porta'] else 'FECHADA'} | "
              f"Tempo Aberta: {tempo_aberto}s | Alarme: {novos_dados['alarme']}")
